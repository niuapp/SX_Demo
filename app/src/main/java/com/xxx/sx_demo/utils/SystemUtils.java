...
public class SystemUtils {
	...
	public static int[] getResolution() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
			...
			try {
				int width = (Integer) Display.class.getMethod("getRawWidth").invoke(defaultDisplay);
				 int height = (Integer) Display.class.getMethod("getRawHeight").invoke(defaultDisplay);
	...
	/**
	 * 判断是否有虚拟按键 NavigationBar
	 * @return
	 */
	public static boolean checkDeviceHasNavigationBar() {
		Context context = UIUtils.getForegroundActivity();
		if (null == context) {
			return false;
		}
